...
const rules = AirValidator.create({
  email: [
    AirValidator.show('请输入你早前注册的邮箱账号').ifEmpty(),
...
async function onReset() {
  await UserService.create(isLoading).resetMyPassword(regVo.value)
  await AirNotification.success('密码重置成功,快去登录吧~')
